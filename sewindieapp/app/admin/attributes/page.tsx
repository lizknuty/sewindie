...
export default function AttributesPage() {
  ...
  const filteredSorted = useMemo(() => {
    ...
    const filtered = q ? attributes.filter((a) => a.name.toLowerCase().includes(q)) : [...attributes]
    switch (sortValue) {
      case "name_desc":
        return filtered.sort((a, b) => b.name.localeCompare(a.name))
      case "patterns_desc":
        return filtered.sort((a, b) => (b._count?.PatternAttribute ?? 0) - (a._count?.PatternAttribute ?? 0))
      case "patterns_asc":
        return filtered.sort((a, b) => (a._count?.PatternAttribute ?? 0) - (b._count?.PatternAttribute ?? 0))
      case "name_asc":
      default:
        return filtered.sort((a, b) => a.name.localeCompare(b.name))
    }
  }, [attributes, query, sortValue])

  const totalAttributes = filteredSorted.length
  const totalPages = Math.max(1, Math.ceil(totalAttributes / rowsPerPage))
  ...
  const rangeStart = totalAttributes === 0 ? 0 : (currentPage - 1) * rowsPerPage + 1
  const rangeEnd = Math.min(currentPage * rowsPerPage, totalAttributes)
  ...
  return (
    <div className="admin-patterns-page">
      <header className="patterns-page-header">
        <div>
          <h1 className="patterns-title">Attributes</h1>
          <p className="patterns-subtitle">Tag patterns with searchable attributes.</p>
        </div>
        <Link href="/admin/attributes/new" className="btn-add-pattern">
          <Plus size={18} />
          Add Attribute
        </Link>
      </header>
      ...
      <div className="patterns-toolbar">
        ...
        <div className="patterns-sort">
          ...
          >
            <option value="name_asc">Attribute Name (A–Z)</option>
            <option value="name_desc">Attribute Name (Z–A)</option>
      ...
      <div className="patterns-resultbar">
        <span className="patterns-count">
          {loading
            ? "Loading attributes..."
            : `Showing ${rangeStart}–${rangeEnd} of ${totalAttributes.toLocaleString()} attributes`}
        </span>
      </div>
      ...
      <div className="patterns-layout filters-hidden">
        <div className="patterns-content">
          {error ? (
            ...
            <div className="patterns-empty">Loading attributes...</div>
          ) : totalAttributes === 0 ? (
            <div className="patterns-empty">No attributes found.</div>
          ) : viewMode === "list" ? (
            <AttributesTable attributes={paginated} />
          ) : (
            <AttributesGrid attributes={paginated} />
          )}

          {!loading && !error && totalAttributes > 0 && (
            <div className="patterns-footer">
              ...
              <nav className="pagination" aria-label="Attribute pages">
                <button
                  type="button"
