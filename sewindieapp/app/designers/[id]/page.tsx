import { query, Designer, Pattern } from '../../lib/db'
...
export default async function DesignerPage({ params }: { params: { id: string } }) {
  try {
    ...
    const patterns = await query('SELECT id, name FROM Pattern WHERE designer_id = $1', [params.id])

    if (designer.rows.length === 0) {
      return <div>Designer not found</div>
    }

    const designerData: Designer = designer.rows[0]

    return (
      <div className="container mx-auto px-4">
        <h1 className="text-3xl font-bold mb-4">{designerData.name}</h1>
        <h2 className="text-2xl font-bold mb-2">Patterns</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {patterns.rows.map((pattern: Pattern) => (
